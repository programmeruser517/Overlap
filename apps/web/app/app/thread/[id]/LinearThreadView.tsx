...
/**
 * maxTurns meaning:
 * - 0 = "Auto" (hard-cap at MAX_TURNS_HARD_CAP)
 * - 2, 4, 6, ... = exact cap (intervals of 2)
 */
function normalizeTurnCap(maxTurns: number) {
  if (maxTurns === 0) return MAX_TURNS_HARD_CAP;
  return Math.max(2, Math.min(MAX_TURNS_HARD_CAP, maxTurns));
}
...
export function LinearThreadView() {
  ...
  // --- thread existence + backfill view_mode when opening in linear view ---
  useEffect(() => {
    if (!id) return;
    fetch(`/api/thread/${id}`, { credentials: "include" })
      ...
      .then((d) => {
        ...
        else if (d.thread.viewMode !== "linear") {
          fetch(`/api/thread/${id}`, {
            method: "PATCH",
            headers: { "Content-Type": "application/json" },
            credentials: "include",
            body: JSON.stringify({ viewMode: "linear" }),
          }).catch(() => {});
        }
      })
  ...
  const runMultiAgents = async (displayTokens: ChatToken[], promptText: string) => {
    ...
    // total messages allowed (includes seed); 0 => auto
    const totalTurns = normalizeTurnCap(maxTurns);
    let turnCount = 1;
    let agreementReachedThisRun = false;
    let agreementContent: string | null = null;
    ...
    try {
      ...
      // Persist thread status in Supabase so it shows in Previous threads and is marked done/cancelled
      if (id) {
        const status = agreementReachedThisRun ? "done" : "cancelled";
        const proposal = agreementReachedThisRun
          ? { summary: agreementContent && agreementContent.length > 200 ? agreementContent.slice(0, 197) + "…" : (agreementContent || "Agreement reached") }
          : undefined;
        fetch(`/api/thread/${id}`, {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
    ...
    <main className="threadPageWrap">
      <header className="threadPageTopbar">
        <div className="threadPageContainer threadPageTopbarInner">
          <Link href="/app?view=linear" className="threadPageBrand">
            ...
            <div className="threadPageRunControls">
              <label className="threadPageRunLabel">
                ...
                <select
                  ...
                  value={maxTurns === 0 || (maxTurns >= 2 && maxTurns <= MAX_TURNS_HARD_CAP && maxTurns % 2 === 0) ? maxTurns : 2}
                  onChange={(e) => setMaxTurns(Number(e.target.value))}
                  disabled={running}
                >
                  <option value={0}>0 (Auto)</option>
                  {Array.from({ length: Math.floor(MAX_TURNS_HARD_CAP / 2) }, (_, i) => 2 * (i + 1)).map((n) => (
      ...
      <div className="threadPageMain">
        {notFound && (
          <p className="threadPageMuted">
            Thread not found.{" "}
            <Link href="/app?view=linear" className="threadPageLink">
              Back to app
            </Link>
          </p>
        )}
                ...
                <div className="threadPageHeaderRow">
                  ...
                  <div className="threadPageHeaderMetaRight">
                    {running ? (
                      ...
                      <span className="threadPageStoppedPill">Done · no agreement</span>
                    ) : (
                      <span className="threadPageCenterHeaderTimer">
                ...
                {messages.length === 0 && !running && !error && (
                  <div className="threadPageCenterDormant">
                    ...
                    <p className="threadPageCenterDormantHint">
                      ...
                      <strong>{MAX_TURNS_HARD_CAP}</strong>).
                    </p>
                  </div>
                )}
              </div>
